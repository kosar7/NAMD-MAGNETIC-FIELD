...
#define SCRIPT_PARSE_STRING(NAME,VAR) { if ( ! strncasecmp(param,(NAME),MAX_SCRIPT_PARAM_SIZE) ) { strcpy(VAR,value); return; } }
  ...
  SCRIPT_PARSE_VECTOR("mField",mField)
  SCRIPT_PARSE_FLOAT("mFieldFreq",mFieldFreq)
  SCRIPT_PARSE_FLOAT("mFieldPhase",mFieldPhase) 
...
// end  additions
   ...
   opts.optionalB("main", "mFieldOn", "Should an magnetic field be applied",
                 &mFieldOn, FALSE);
   opts.optionalB("mFieldOn", "mFieldNormalized", "Is mField vector scaled by cell basis vectors?",
                 &mFieldNormalized, FALSE);
   opts.require("mFieldOn", "mField", "Magnetic field vector", &mField);
   opts.optional("mFieldOn", "mFieldFreq", "Magnetic field frequency", &mFieldFreq);
   opts.optional("mFieldOn", "mFieldPhase", "Magnetic field phase", &mFieldPhase);
...
//fepe
  ...
  sphericalBCexp2 = 0;
   ...
   {
   	if (!opts.defined("mFieldFreq")) mFieldFreq = 0.0;
	if (!opts.defined("mFieldPhase")) mFieldPhase = 0.0;
   }
...
//         iout << iINFO << "QM MULTIPLICITY: " << qmMult << "\n";
  ...
  if (LCPOOn) {
   ...
   if (mFieldOn)
   {
      iout << iINFO << "MAGNETIC FIELD ACTIVE\n";
      
      iout << iINFO << "M-FIELD VECTOR         ("
         << mField.x << ", " << mField.y
         << ", " << mField.z << ")\n";
      if ( mFieldNormalized ) iout << iINFO << "M-FIELD VECTOR IS SCALED BY CELL BASIS VECTORS\n";
      iout << iINFO << "M-FIELD FREQUENCY IS (1/ps) " << mFieldFreq << "\n";
      iout << iINFO << "M-FIELD PHASE IS     (deg)  " << mFieldPhase << "\n";
